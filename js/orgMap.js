window.renderOrgMap = function () {
    ...
    toolbar.innerHTML = `
        <div class="org-legend">
            <span class="legend-item"><span class="dot root"></span> ${window.t ? window.t('root') : 'Root'}</span>
            <span class="legend-item"><span class="dot area"></span> ${window.t ? window.t('orgArea') : 'Área'}</span>
            <span class="legend-item"><span class="dot person"></span> ${window.t ? window.t('orgPerson') : 'Pessoa'}</span>
        </div>
        <div style="display: flex; gap: 10px; align-items: center;">
            <p class="org-hint" style="margin: 0; margin-right: 15px;">${window.t ? window.t('orgHint') : '* Clique no nome para editar...'}</p>
            <button class="btn-save-org" style="background: var(--status-blue); border: none; padding: 6px 16px; border-radius: 4px; color: #001f3f; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 6px;">
