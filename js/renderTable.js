...
window.renderTable = function (sectionData, sectionKey) {
    ...
    sectionData.rows.forEach((rowData, index) => {
        ...
        keysToRender.forEach(key => {
            ...
            if (key === 'startDate' || key === 'endDate') {
                ...
                // Convert DD/MM/AAAA to AAAA-MM-DD for the native date input
                ...
                if (sectionKey === 'initiatives') {
                    ...
                    statusSelect.style.backgroundColor = getStatusColorClass(cellValue) === 'green' ? 'rgba(46, 204, 64, 0.3)' :
                        getStatusColorClass(cellValue) === 'orange' ? 'rgba(255, 133, 27, 0.3)' :
                            getStatusColorClass(cellValue) === 'blue' || cellValue === 'planned' || cellValue === 'Planejado' ? 'rgba(0, 116, 217, 0.3)' : // Blue for planned
    ...
    btnAdd.style.backgroundColor = '#2ecc40'; // Green
    btnAdd.style.color = 'white';
    btnAdd.style.border = 'none';
    btnAdd.style.borderRadius = '4px';
    btnAdd.style.float = 'right'; // Align to right
